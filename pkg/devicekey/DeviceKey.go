...
func CalculateUUID(BuildTime string, envDecryptKey string, envFirstKey string) error {

	requiredDate := time.Date(2025, time.April, 1, 0, 0, 0, 0, time.Local)
	currentDate := time.Now()
	if currentDate.After(requiredDate) {
		return errors.New("Error: expired session. Please, active device again")
	}

	// Đọc đường dẫn SSH private key từ argument
	sshKeyPath := flag.String("ssh-key", "", "Path to SSH private key")
	flag.Parse()

	if *sshKeyPath == "" {
		return errors.New("Error: Please provide the path to the SSH private key using the -ssh-key argument")
	}
	...
	// envDecryptKey := "myEnv@Iiquo2DKey"
