...
// Tạo mới `ShardelDB`
func NewShardelDB(baseDir string, numShards int, parallelism int, dbType DBType) (*ShardelDB, error) {
...
// BatchPut: Ghi nhiều key-value vào đúng shard song song, nhóm theo shard trước khi gọi shard.BatchPut
func (s *ShardelDB) BatchPut(kvs [][2][]byte) error {
	var wg sync.WaitGroup
	errChan := make(chan error, s.numShards) // Kênh để thu thập lỗi

	// Nhóm kvs theo shard
	shardBatches := make(map[int][][2][]byte)
	for _, kv := range kvs {
		shardIndex := int(s.getShardIndex(kv[0])) // Sử dụng hàm getShardIndex để lấy index
	...
	for shardIndex, batch := range shardBatches {
		...
		go func(shardIndex int, batch [][2][]byte) {
			...
			if shard == nil {
				errChan <- fmt.Errorf("shard %d is nil", shardIndex) //Thêm kiểm tra nil
				return
			}
			if len(batch) == 0 {
				return // bỏ qua batch rỗng
			}
			if err := shard.BatchPut(batch); err != nil {
				errChan <- fmt.Errorf("shard %d: batch: %v, error: %w", shardIndex, batch, err) // Bao gồm shardIndex và batch trong lỗi
			}
		}(shardIndex, batch)
	}
