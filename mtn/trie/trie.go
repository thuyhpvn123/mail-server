...
func New(root e_common.Hash, db trie_db.DB) (*MerklePatriciaTrie, error) {
...
func (t *MerklePatriciaTrie) Get(key []byte) ([]byte, error) {
	...
	value, newroot, didResolve, err := t.get(t.root, node.KeybytesToHex(hashedKey), 0)
...
func (t *MerklePatriciaTrie) insert(
	...
	case node.HashNode:
		logger.DebugP("hash node", hex.EncodeToString(n))
...
func (t *MerklePatriciaTrie) Update(key, value []byte) error {
	...
	hashedKey := crypto.Keccak256(key)
	return t.update(hashedKey, value)
}
...
func GetRootHash(
	...
	trie, err := New(e_common.Hash{}, memStorage)
	if err != nil {
